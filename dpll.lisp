...
(defun cnfdesc (c)
  (let ((nc (length c)))
    (if (> nc 10)
        (list 'num-clauses nc 'num-literals (length (apply #'append c)))
        c)))
...
(defun sudoku-field (row line)
  (let (res)
    (dotimes (num 9)
      (push (sudoku-var row line num) res))
    res))
...
(defun sudoku-row (row num)
  (let (res)
    (dotimes (y 9)
      (push (sudoku-var row y num) res))
    res))
...
(defun sudoku ()
  (let (res)
    (dotimes (line 9)
      (dotimes (row 9)
        (push (one-of (sudoku-field row line)) res)))
    (dotimes (line 9)
      (dotimes (num 9)
        (push (one-of (sudoku-line line num)) res)))
    (dotimes (row 9)
      (dotimes (num 9)
        (push (one-of (sudoku-row row num)) res)))
    (dotimes (br 3)
      (dotimes (bl 3)
        (dotimes (num 9)
...
(defun sudoku-game (fields)
  (let ((row 0) (line 0) cl res)
    (dolist (f fields)
      (when (integerp f)
        (push (list (sudoku-var row line (- f 1))) res))
      (incf row)
      (when (= row 9)
        (setq row 0)
        (incf line)))
